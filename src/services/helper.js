class Helper {
  ...
  parseData(data) {
    let parseType = this._checkData(data);
    return parseType(data);
  }

  _checkData(data) {
    if (this.hasProp(data, "artists")) return this.parseArtists;
    else if (this.hasProp(data, "albums")) return this.parseAlbums;
    else if (this.hasProp(data, "tracks")) return this.parseTracks;
  }
